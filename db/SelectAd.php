        ...
        <div class="col-sm-6">
            <div class="card">
                <div class="card-block">
                    <a href="marketplace/ad/<?=$row['id_ad'] ?>">
                        <?php
                if (!empty($row['image_name']))
                { ?>
                        <img width="100" height="100" src="marketplace/img/submitted/<?=$row['image_name'] ?>"
                            class="rounded float-left img-responsive border border-dark"></a>
                    <?php
                }
                else
                { ?>
                    <img width="100" height="100" src="marketplace/img/question_mark.png"
                        class="rounded float-left img-responsive border border-dark"></a>
                    <?php
                } ?>
                    <a class="card-title" href='marketplace/ad/<?=$row['id_ad'] ?>'><?=$row['title'] ?></a>
...
?>
        <h2>Nothing to show yet
            <a href='marketplace/index.php'></h2>
    ...
    public function DisplaySingleAd($query)
    {
        ...
        if ($count > 0)
        {
            ...
            foreach ($result as $row)
            {
                ...
                if (!empty($row['image_name']))
                {
                    echo '<img class="single-img img-fluid border" src= "marketplace/img/submitted/' . $row['image_name'] . '"</br>';
                }
                else
                {
                    echo '<img class="single-img img-fluid border" src= "marketplace/img/question_mark.png "' . '</br>';
                }
?>
<h1> <?=$row['title'] ?> </h1>
